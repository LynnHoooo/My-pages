"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.reverseMigrateOptions = exports.addOldOptions = void 0;
var tslib_1 = require("tslib");
var optionsUtils_1 = require("./optionsUtils");
var layout_1 = require("../../common/constants/layout");
var optionsMap_1 = tslib_1.__importDefault(require("./optionsMap"));
var migratorStore_1 = require("./migratorStore");
function addOldOptions(flatOptions) {
    var flat_migrated = reverseMigrateOptions(flatOptions);
    var flat_combinedOptions = tslib_1.__assign(tslib_1.__assign({}, optionsUtils_1.trimUndefinedValues_flat(flat_migrated)), optionsUtils_1.trimUndefinedValues_flat(flatOptions));
    return flat_combinedOptions;
}
exports.addOldOptions = addOldOptions;
function reverseMigrateOptions(flatOptionsObject) {
    var oldOptions = tslib_1.__assign({}, flatOptionsObject);
    ///----------- LAYOUT -------------///
    oldOptions = migratorStore_1.changeNames(oldOptions, tslib_1.__spreadArrays(migratorStore_1.nameChangedLayoutParams).map(function (ele) { return tslib_1.__spreadArrays(ele).reverse(); }));
    oldOptions = migratorStore_1.reverseBooleans(oldOptions, tslib_1.__spreadArrays(migratorStore_1.reversedLayoutParams).map(function (ele) { return tslib_1.__spreadArrays(ele).reverse(); }));
    oldOptions = process_new_to_old_ThumbnailAlignment(oldOptions);
    oldOptions = process_new_to_old_ScrollDirection(oldOptions);
    oldOptions = process_new_to_old_LayoutOrientation(oldOptions);
    oldOptions = process_new_to_old_groupsOrder(oldOptions);
    oldOptions = process_new_to_old_responsiveMode(oldOptions);
    oldOptions = process_new_to_old_NumberOfColumns(oldOptions); // fixedColumns || numberOfImagesPerRow ||
    oldOptions = process_new_to_old_targetItemSizeUnit(oldOptions);
    oldOptions = process_new_to_old_targetItemSizeValue(oldOptions);
    oldOptions = process_new_to_old_CroppedAlignment(oldOptions);
    oldOptions = process_new_to_old_CropRatio(oldOptions);
    oldOptions = process_new_to_old_textBoxSizeMode(oldOptions);
    oldOptions = process_new_to_old_columnRatios(oldOptions);
    oldOptions = process_new_to_old_cropMethod(oldOptions);
    oldOptions = process_new_to_old_GroupTypes(oldOptions);
    oldOptions = process_new_to_old_AllowedGroupTypes(oldOptions);
    oldOptions = process_new_to_old_gallerySpacing(oldOptions);
    oldOptions = process_new_to_old_slideshowInfoSize(oldOptions);
    oldOptions = process_new_to_old_arrowsPosition(oldOptions);
    ///----------- BEHAVIOUR -------------///
    oldOptions = migratorStore_1.changeNames(oldOptions, tslib_1.__spreadArrays(migratorStore_1.nameChangedBehaviourParams).map(function (ele) { return tslib_1.__spreadArrays(ele).reverse(); }));
    oldOptions = migratorStore_1.reverseBooleans(oldOptions, tslib_1.__spreadArrays(migratorStore_1.reversedBehaviourParams).map(function (ele) { return tslib_1.__spreadArrays(ele).reverse(); }));
    oldOptions = process_new_to_old_ClickAction(oldOptions);
    oldOptions = process_new_to_old_VideoPlayTrigger(oldOptions);
    oldOptions = process_new_to_old_VideoVolume(oldOptions);
    oldOptions = process_new_to_old_VideoSpeed(oldOptions);
    oldOptions = process_new_to_old_OverlayHoveringBehaviour(oldOptions);
    oldOptions = process_new_to_old_InfoPlacement(oldOptions);
    oldOptions = process_new_to_old_layoutDirection(oldOptions);
    oldOptions = process_new_to_old_LoadMoreAmount(oldOptions);
    oldOptions = process_new_to_old_AutoSlideBehaviour(oldOptions);
    oldOptions = process_new_to_old_galleryTextAlign(oldOptions);
    ///----------- STYLING -------------///
    oldOptions = migratorStore_1.changeNames(oldOptions, tslib_1.__spreadArrays(migratorStore_1.nameChangedStylingParams).map(function (ele) { return tslib_1.__spreadArrays(ele).reverse(); }));
    return oldOptions;
}
exports.reverseMigrateOptions = reverseMigrateOptions;
//----- refactor functions ----------//
function process_new_to_old_ThumbnailAlignment(obj) {
    var _a;
    //['galleryThumbnailsAlignment', optionsMap.layoutParams.thumbnails.alignment'],
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.thumbnails.alignment, 'galleryThumbnailsAlignment');
    _obj['galleryThumbnailsAlignment'] = (_a = _obj['galleryThumbnailsAlignment']) === null || _a === void 0 ? void 0 : _a.toLowerCase();
    return _obj;
}
function process_new_to_old_targetItemSizeUnit(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.targetItemSize.unit, 'gallerySizeType');
    switch (_obj['gallerySizeType']) {
        case 'PIXEL':
            _obj['gallerySizeType'] = 'px';
            break;
        case 'PERCENT':
            _obj['gallerySizeType'] = 'ratio';
            break;
        case 'SMART':
            _obj['gallerySizeType'] = 'smart';
            break;
    }
    return _obj;
}
function process_new_to_old_targetItemSizeValue(obj) {
    var _obj = obj;
    var value = _obj[optionsMap_1.default.layoutParams.targetItemSize.value];
    var type = _obj['gallerySizeType'];
    var keys = {
        smart: 'gallerySize',
        px: 'gallerySizePx',
        ratio: 'gallerySizeRatio',
    };
    _obj['gallerySize'] =
        _obj['gallerySizePx'] =
            _obj['gallerySizeRatio'] =
                undefined;
    _obj[keys[type]] = value;
    delete _obj[optionsMap_1.default.layoutParams.targetItemSize.value];
    return _obj;
}
function process_new_to_old_slideshowInfoSize(obj) {
    var _a, _b;
    var _obj = obj;
    var isSlideshow = layout_1.isLayout('SLIDESHOW')({
        galleryLayout: obj.galleryLayout >= -3
            ? obj.galleryLayout
            : (_b = (_a = obj.layoutParams) === null || _a === void 0 ? void 0 : _a.structure) === null || _b === void 0 ? void 0 : _b.galleryLayout,
    });
    if (isSlideshow) {
        _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.info.height, 'slideshowInfoSize');
        _obj['textBoxHeight'] = _obj['slideshowInfoSize'];
    }
    else {
        _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.info.height, 'textBoxHeight');
        _obj['slideshowInfoSize'] = _obj['textBoxHeight'];
    }
    return _obj;
}
function process_new_to_old_textBoxSizeMode(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.info.sizeUnits, 'calculateTextBoxWidthMode');
    switch (_obj['calculateTextBoxWidthMode']) {
        case 'PERCENT':
            _obj['calculateTextBoxWidthMode'] = 'PERCENT';
            _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.info.width, 'textBoxWidthPercent');
            _obj['textBoxWidth'] = _obj['textBoxWidth'] || 200;
            break;
        case 'PIXEL':
            _obj['calculateTextBoxWidthMode'] = 'MANUAL';
            _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.info.width, 'textBoxWidth');
            _obj['textBoxWidthPercent'] = _obj['textBoxWidthPercent'] || 50;
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_arrowsPosition(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.navigationArrows.position, 'arrowsPosition');
    switch (_obj['arrowsPosition']) {
        case 'ON_GALLERY':
            _obj['arrowsPosition'] = 0;
            break;
        case 'OUTSIDE_GALLERY':
            _obj['arrowsPosition'] = 1;
            break;
        case 'MOUSE_CURSOR':
            _obj['arrowsPosition'] = 2;
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_responsiveMode(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.structure.responsiveMode, 'gridStyle');
    switch (_obj['gridStyle']) {
        case 'FIT_TO_SCREEN':
            _obj['gridStyle'] = 0;
            break;
        case 'SET_ITEMS_PER_ROW':
            _obj['gridStyle'] = 1;
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_columnRatios(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.structure.columnRatios, 'columnWidths');
    _obj['columnWidths'] = _obj['columnWidths']
        ? _obj['columnWidths'].join
            ? _obj['columnWidths'].join(',')
            : ''
        : undefined;
    return _obj;
}
function process_new_to_old_cropMethod(obj) {
    var _a;
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.crop.method, 'cubeType');
    _obj['cubeType'] = (_a = _obj['cubeType']) === null || _a === void 0 ? void 0 : _a.toLowerCase();
    return _obj;
}
function process_new_to_old_VideoPlayTrigger(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.behaviourParams.item.video.playTrigger, 'videoPlay');
    switch (_obj['videoPlay']) {
        case 'CLICK':
            _obj['videoPlay'] = 'onClick';
            break;
        case 'HOVER':
            _obj['videoPlay'] = 'hover';
            break;
        case 'AUTO':
            _obj['videoPlay'] = 'auto';
            break;
        default:
            break;
    }
    return _obj;
}
// function process_old_to_new_targetItemSizeMode(obj) {
//   let _obj = obj;
//   _obj = namingChange(
//     _obj,
//     'gallerySizeType',
//     optionsMap.layoutParams.targetItemSize.mode
//   );
//   _obj.layoutParams.targetItemSize.mode =
//     _obj.layoutParams.targetItemSize.mode?.toUpperCase();
//   return _obj;
// }
function process_new_to_old_VideoVolume(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.behaviourParams.item.video.volume, 'videoSound');
    _obj['videoSound'] =
        typeof _obj['videoSound'] === 'undefined'
            ? undefined
            : !!_obj['videoSound'];
    return _obj;
}
function process_new_to_old_VideoSpeed(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.behaviourParams.item.video.speed, 'videoSpeed');
    _obj['videoSpeed'] =
        _obj['videoSpeed'] >= 0 ? String(_obj['videoSpeed']) : undefined;
    return _obj;
}
function process_new_to_old_gallerySpacing(obj) {
    var _obj = obj;
    _obj['layoutParams_gallerySpacing'] = _obj['galleryMargin'] =
        _obj['layoutParams_structure_gallerySpacing'];
    delete _obj['layoutParams_structure_gallerySpacing'];
    return _obj;
}
function process_new_to_old_ScrollDirection(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.structure.scrollDirection, 'scrollDirection');
    switch (_obj['scrollDirection']) {
        case 'VERTICAL':
            _obj['scrollDirection'] = 0;
            break;
        case 'HORIZONTAL':
            _obj['scrollDirection'] = 1;
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_layoutDirection(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.behaviourParams.gallery.layoutDirection, 'isRTL');
    switch (_obj['isRTL']) {
        case 'RIGHT_TO_LEFT':
            _obj['isRTL'] = true;
            break;
        case 'LEFT_TO_RIGHT':
            _obj['isRTL'] = false;
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_LayoutOrientation(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.structure.layoutOrientation, 'isVertical');
    switch (_obj['isVertical']) {
        case 'VERTICAL':
            _obj['isVertical'] = true;
            break;
        case 'HORIZONTAL':
            _obj['isVertical'] = false;
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_groupsOrder(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.structure.groupsOrder, 'placeGroupsLtr');
    switch (_obj['placeGroupsLtr']) {
        case 'LEFT_TO_RIGHT':
            _obj['placeGroupsLtr'] = true;
            break;
        case 'RIGHT_TO_LEFT':
        case 'BY_HEIGHT':
            _obj['placeGroupsLtr'] = false;
            break;
        default:
            _obj['placeGroupsLtr'] = undefined;
    }
    // 'RIGHT_TO_LEFT' doesnt exist yet.
    return _obj;
}
function process_new_to_old_galleryTextAlign(obj) {
    var _a;
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.behaviourParams.gallery.horizontal.slideshowInfo
        .buttonsAlignment, 'galleryTextAlign');
    _obj['galleryTextAlign'] = (_a = _obj['galleryTextAlign']) === null || _a === void 0 ? void 0 : _a.toLowerCase();
    return _obj;
}
function process_new_to_old_LoadMoreAmount(obj) {
    var _a;
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.behaviourParams.gallery.vertical.loadMore.amount, 'loadMoreAmount');
    _obj['loadMoreAmount'] = (_a = _obj['loadMoreAmount']) === null || _a === void 0 ? void 0 : _a.toLowerCase();
    return _obj;
}
function process_new_to_old_CroppedAlignment(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.crop.alignment, 'cubeFitPosition');
    switch (_obj['cubeFitPosition']) {
        case 'CENTER':
            _obj['cubeFitPosition'] = 'MIDDLE';
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_OverlayHoveringBehaviour(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.behaviourParams.item.overlay.hoveringBehaviour, 'hoveringBehaviour');
    switch (_obj['hoveringBehaviour']) {
        case 'ALWAYS_VISIBLE':
            _obj['hoveringBehaviour'] = 'NO_CHANGE';
            break;
        case 'NEVER_VISIBLE':
            _obj['hoveringBehaviour'] = 'NEVER_SHOW';
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_InfoPlacement(obj) {
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.layoutParams.info.placement, 'titlePlacement');
    switch (_obj['titlePlacement']) {
        case 'OVERLAY':
            _obj['titlePlacement'] = 'SHOW_ON_HOVER';
            break;
        case 'BELOW':
            _obj['titlePlacement'] = 'SHOW_BELOW';
            break;
        case 'ABOVE':
            _obj['titlePlacement'] = 'SHOW_ABOVE';
            break;
        case 'RIGHT':
            _obj['titlePlacement'] = 'SHOW_ON_THE_RIGHT';
            break;
        case 'LEFT':
            _obj['titlePlacement'] = 'SHOW_ON_THE_LEFT';
            break;
        case 'ALTERNATE_HORIZONTALLY':
            _obj['titlePlacement'] = 'ALTERNATE_HORIZONTAL';
            break;
        case 'ALTERNATE_VERTICALLY':
            _obj['titlePlacement'] = 'ALTERNATE_VERTICAL';
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_ClickAction(obj) {
    var _a;
    var _obj = obj;
    _obj = migratorStore_1.namingChange(_obj, optionsMap_1.default.behaviourParams.item.clickAction, 'itemClick');
    _obj['itemClick'] = (_a = _obj['itemClick']) === null || _a === void 0 ? void 0 : _a.toLowerCase();
    switch (_obj['itemClick']) {
        case 'action':
            _obj['itemClick'] = 'expand';
            break;
        default:
            break;
    }
    return _obj;
}
function process_new_to_old_AutoSlideBehaviour(obj) {
    var _obj = obj;
    var val = _obj[optionsMap_1.default.behaviourParams.gallery.horizontal.autoSlide.behaviour];
    switch (val) {
        case 'OFF':
            _obj.isAutoSlideshow = false;
            _obj.autoSlideshowType = 'interval';
            break;
        case 'INTERVAL':
            _obj.isAutoSlideshow = true;
            _obj.autoSlideshowType = 'interval';
            break;
        case 'CONTINUOUS':
            _obj.isAutoSlideshow = true;
            _obj.autoSlideshowType = 'continuous';
            break;
    }
    delete _obj[optionsMap_1.default.behaviourParams.gallery.horizontal.autoSlide.behaviour];
    return _obj;
}
function process_new_to_old_CropRatio(obj) {
    var _obj = obj;
    var rotatingCropRatioVal;
    var val = _obj[optionsMap_1.default.layoutParams.crop.ratios] && tslib_1.__spreadArrays(_obj[optionsMap_1.default.layoutParams.crop.ratios]);
    var newVal = val || [];
    if (newVal.length > 1) {
        rotatingCropRatioVal = newVal.slice(0).join(',');
    }
    else if ((val === null || val === void 0 ? void 0 : val.length) === 0) {
        rotatingCropRatioVal = '';
    }
    else {
        rotatingCropRatioVal = undefined;
    }
    _obj['layoutParams_cropRatio'] = _obj['cubeRatio'] = newVal[0];
    _obj.rotatingCropRatios = rotatingCropRatioVal;
    delete _obj[optionsMap_1.default.layoutParams.crop.ratios];
    return _obj;
}
function process_new_to_old_AllowedGroupTypes(obj) {
    var _obj = obj;
    _obj['groupTypes'] = _obj[optionsMap_1.default.layoutParams.groups.allowedGroupTypes]
        ? _obj[optionsMap_1.default.layoutParams.groups.allowedGroupTypes].join
            ? _obj[optionsMap_1.default.layoutParams.groups.allowedGroupTypes].join(',')
            : ''
        : undefined;
    delete _obj[optionsMap_1.default.layoutParams.groups.allowedGroupTypes];
    return _obj;
}
function process_new_to_old_GroupTypes(obj) {
    var _obj = obj;
    var repeatingVal = _obj[optionsMap_1.default.layoutParams.groups.repeatingGroupTypes] &&
        _obj[optionsMap_1.default.layoutParams.groups.repeatingGroupTypes].join(',');
    _obj['layoutParams_repeatingGroupTypes'] = _obj['rotatingGroupTypes'] =
        repeatingVal;
    delete _obj[optionsMap_1.default.layoutParams.groups.repeatingGroupTypes];
    return _obj;
}
function process_new_to_old_NumberOfColumns(obj) {
    var _obj = obj;
    _obj.fixedColumns = _obj.fixedColumns || undefined;
    _obj.numberOfImagesPerRow =
        _obj[optionsMap_1.default.layoutParams.structure.numberOfColumns];
    delete _obj[optionsMap_1.default.layoutParams.structure.numberOfColumns];
    return _obj;
}
//# sourceMappingURL=optionsBackwardConverter.js.map