...
var Blueprints = /** @class */ (function () {
    ...
    Blueprints.prototype.createBlueprint = function (_a) {
        ...
        var reasons = Object.entries(this.reasons)
            ...
            return tslib_1.__spreadArrays(reasons, [param + ": " + reason]);
        }, [])
            .join(', ');
    ...
    Blueprints.prototype.formatItemsIfNeeded = function (items, lastItems) {
        ...
        if (itemsWereAdded(items, oldItemsParams)) {
            formattedItems = oldItemsParams.concat(items.slice(oldItemsParams.length).map(function (item) {
                return pro_layouts_1.ItemsHelper.convertDtoToLayoutItem(item);
            }));
            ...
            formattedItems = items.map(function (item) {
                return Object.assign(pro_layouts_1.ItemsHelper.convertDtoToLayoutItem(item));
            });
    ...
    Blueprints.prototype.formatOptionsIfNeeded = function (options, lastOptions, isUsingCustomInfoElements) {
        ...
        if (optionsHaveChanged(options, oldOptions)) {
            var mergedOldAndNewStyles = pro_gallery_lib_1.extendNestedOptionsToIncludeOldAndNew(options); //add both old and new options
            var fullOptionsOverDefualts = pro_gallery_lib_1.populateWithDefaultOptions(mergedOldAndNewStyles); //add default for any undefined option
            formattedOptions = pro_gallery_lib_1.extendNestedOptionsToIncludeOldAndNew(pro_gallery_lib_1.processLayouts(pro_gallery_lib_1.addPresetOptions(fullOptionsOverDefualts), isUsingCustomInfoElements)); // TODO make sure the processLayouts is up to date. delete addLayoutStyles from layoutsHelper when done with it...
            changed = true;
        }
    ...
    Blueprints.prototype.formatContainerIfNeeded = function (container, lastContainer, lastOptions, formattedOptions, optionsChanged) {
        ...
        if (optionsChanged || // If options changed they could affect the container and a new container must be created (slideshow,thumbs,shadow,borders...etc)
            ...
            changed = true;
            formattedContainer = Object.assign({}, container, pro_gallery_lib_1.dimensionsHelper.getGalleryDimensions());
        }
    ...
    Blueprints.prototype.createStructure = function (_a) {
        ...
        this.layouter = new pro_layouts_1.Layouter(layoutParams); // TODO - no need for "this."
        // }
